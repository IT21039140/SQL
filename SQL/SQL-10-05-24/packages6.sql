...
--  updated in 22-12-23
  ...
  FUNCTION GET_SEARCHED_AUDIT_CASE_RM (
    s_audit_area IN VARCHAR2, 
    s_audit_mode IN VARCHAR2,
    s_year IN NUMBER,
    s_overall_audit_status IN VARCHAR2,
    s_audit_div IN VARCHAR2, 
    s_refNum IN VARCHAR2
) RETURN AUDIT_CASE_TYPE_LIST;
...
FUNCTION GET_SEARCHED_AUDIT_CASE_RM (
    ...
    s_audit_div IN VARCHAR2, 
    s_refNum IN VARCHAR2
) RETURN AUDIT_CASE_TYPE_LIST AS
    CURSOR MAIN_CASE_CUR IS
        SELECT *
        FROM AUDIT_CASE_TBL
        WHERE AUDIT_AREA = s_audit_area
        ...
        AND YEAR = s_year
        AND (s_overall_audit_status IS NULL OR OVERALL_AUDIT_STATUS = s_overall_audit_status)
        AND (s_audit_div IS NOT NULL AND AUDIT_DIV = s_audit_div)
        AND (s_refNum IS NULL OR AUDIT_REPORT_REF_NUM = s_refNum);
...
BEGIN
    AUDIT_CASE_LIST := AUDIT_CASE_TYPE_LIST();
    FOR MAIN_CASE_CUR_REC IN MAIN_CASE_CUR LOOP
        AUDIT_CASE := NEW AUDIT_CASE_TYPE(
    ...
    RETURN AUDIT_CASE_LIST;
END GET_SEARCHED_AUDIT_CASE_RM;
...
BEGIN
    ...
    ELSE
        ...
        updateStatus := 'Fail to update main cese in data base!';
    END IF;
...
--    COMMIT;
  IF sql%rowcount > 0 THEN
    ...
    ELSE
        dbms_output.put_line('Update failed: No rows affected');
        add_status := 'Fail to add Overall RM commnet in data base';
    END IF;
...
BEGIN
    ...
    OPEN cursor1 FOR
        ...
        FROM AUDIT_INCIDENT_TBL
        WHERE MAIN_CASE_ID = S_main_case_id AND Del_Flag!=1 AND AUDIT_STATUS!=0 AND AUDIT_STATUS!=2;--update at 27-12-23
